...
def enable_modem():
    os.system("/home/nsl71/ofono/test/enable-modem")
...
def run_minimodem(bps, timeout_val):
...
def derive_key():
    ...
    while(True):
        ...
        if(result.count(b'=') >= 1):
            ...
            mute_sink()
            break
    print(f"[--] Time taken for sending key in one direction {time.time()-ts}")
    while(True):
        mute_sink()
        proc = run_minimodem(8, 70)
        proc1 = run_minimodem(8, 14)
