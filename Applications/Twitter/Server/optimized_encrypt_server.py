    ...
    for i,chunk in enumerate(data_chunks):                                                                                                                            
        ...
        if(len(chunk) == 28):                                                                                                                                       
            try:                                                                                                                                                    
                ...
                continue
            print(f"[+] Decoded Content: {decoded_data} {decoded_data[:16]}")
            print(aes_obj)
...
def derive_key():
    ...
    while(True):
        ...
        proc1 = run_minimodem("hello", 16, 7)
        result = proc1.communicate()[0]
        result = result.replace(b'\n',b'')
        print(f"[+++]Received data: {result}")
        if(result.count(b'=') >= 3 or got_public):
            proc.kill()
            break

        result = proc.communicate()[0]
        result = result.replace(b'\n',b'')
        print(f"[+++] Length : {len(result)} Received data: {result}")
...
def run_test(modems , out_file , bps):
    ...
    while(data_left):
        ...
        if(missing_str == "00000"):
            ...
            # TWITTER CODE
            twitter = Twython(app_key=<app_key>, app_secret=<app_secret>, oauth_token=<oauth_token>, oauth_token_secret=<oauth_token_secret>, access_token=<access_token>)
